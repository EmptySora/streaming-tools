...
#XXX version 1.1.0.1: changes to allow easy modifications for github (plus a few bug fixes)
...
CPY_PATH = "PATH TO COMMAND_PY HERE"      #With trailing backslash (\)
BURLS_FILE = "PATH TO BANNED URLS FILE"   #One url per line
USER_NAME = "YOUR USERNAME"               #All lowercase
USER_ID = 0                               #You'd best look up how to get this...
API_KEY = "YOUR TWITCH CLIENT ID/API KEY" #Look this up too...
...
def LoadAppSettings():
    ...
    if not os.path.isfile(CPY_PATH + "settings.json"):
        return # No settings file, quit
    contents = ""
    with open(CPY_PATH + "settings.json", "r") as fh:
        contents = fh.read()
...
def CreateSettingsJSON():
    ...
    with open(CPY_PATH + "Generated_UI_Config.json", "w") as fh:
        fh.write(config)
    return
...
def GetBlockedURLS():
    with open(BURLS_FILE) as fh:
        return [line.strip() for line in fh]

def BlockURL(url):
    with open(BURLS_FILE, "a") as fh:
        fh.write(url + "\r\n")
    return
...
def OldStreamStatus():
    if Parent.IsLive():
        try:
            stream = HelixAPI("GET", "streams", {"user_id": USER_ID}, None)
...
def jcurl(m, u, q, p, h):
    #Parent.Log(ScriptName, "Bro do you even jcurl?")
    string = curl(m, u, q, p, h)
    if string is None:
        return None
    #Parent.Log(ScriptName, "So you do: " + string)
...
def Followage(requestingUser, targetUser, rUserID = False, tUserID = False, requestingUserName = None, targetUserName = None):
    ...
    if not rUserID:
        ...
        if not found_user:
            return {
                "error": True,
                "message": "User \"" + str(requestingUser) + "\" does not exist!"
                }
    if not tUserID:
        #get user id
        ...
        if not found_user:
            return {
                "error": True,
                "message": "User \"" + str(targetUser) + "\" does not exist!"
                }
...
def LoadCommands():
    ...
    #                               Function Delegate     Name           Command           CD  UCD  Permissions  Info  Cost  Enabled  Command Usage
    ...
    #the commented lines above are being replaced with the following:
    #!8ball: "$dummy $username: $readrandline(C:\stream_resources\BOT\8ball.txt)"
    #!uptime: "$username: The stream has been live for: $uptime"
    #!pun: "$readrandline(C:\stream_resources\sorabot\puns.txt)"
    #!so: "$dummy One shout-out for a wonderful $targetname, who was last playing "$game": Pay them a visit at $url!"
    #doing this because we can actually run these four commands (and probably even more) using pure command syntax in SLCB


    #C:\stream_resources is a shortcut to "%userprofile%\Documents\EmptySora (Stream Resources)\"
    #C:\stream_resources\sorabot shortcut to "%userprofile%\Documents\SoraBot\"
    return

def GetUIElement(prop, command, gname):
    cvalue = lambda x, y: eval("x." + y)
    t = prop["type"]
    ret = {
        "type": t,
        "value": cvalue(command, prop["name"]),
        "label": prop["label"],
    ...
    if t == "dropdown":
        ret["items"] = []
        for k, v in prop["items"].iteritems():
            if v == ret["value"]:
                ret["value"] = k
            ret["items"].append(k)
    elif t == "slider":
...
def GenerateConfiguration():
    ...
    cname = lambda x: x.CommandName
    props = []
    props.append({
        "type": "textbox",
        "name": "Command",
        "label": "Command Usage",
        "tooltip": "The name of the command that triggers this function."
        })
    props.append({
        "type": "checkbox",
        "name": "Enabled",
        "label": "Enabled",
        "tooltip": "Enable the command to be used in chat?"
        })
    props.append({
        "type": "slider",
        "name": "Cooldown",
        "label": "Cooldown (seconds)",
        "tooltip": "The amount of time that is required to pass in between successful calls of the command.",
        "min": 0,
        "max": 60*60,
        "ticks": 1
        })
    props.append({
        ...
        "tooltip": "The amount of time that is required to pass in between successful calls of the command by a particular user.",
        "min": 0,
        "max": 60*60,
        "ticks": 1
        })
    props.append({
        ...
        "name": "Cost",
        "label": "Cost",
        "tooltip": "The amount of currency required to run the command."
        })
    props.append({
        "type": "dropdown",
        "name": "Permission",
        "label": "Permission",
        "tooltip": "The required level of permission needed to run the command",
        "items": listdict([
            "everyone", "regular", "subscriber", "gamewisp subscriber",
            "user_specific", "min_rank", "min_points", "min_hours",
            "moderator", "editor", "caster"
            ])
        })
    props.append({
        "type": "textbox",
        "name": "Info",
        "label": "Info",
        "tooltip": "The secondary argument to the permission checking, only required for certain values of permission."
        })
    props.append({
        "type": "dropdown",
        "name": "UsageMask",
        "label": "Usage",
        "tooltip": "Where the command is allowed to be executed",
        "items": {
            "Twitch Chat": 5,
            "Twitch Whisper": 9,
            "Discord Chat": 6,
    ...
    for command in CommandSet:
        for prop in props:
            ui["COMPY_" + prop["type"] + "_" + cname(command) + "_" + prop["name"]] = GetUIElement(prop, command, cname(command))
    #add a UI button that can trigger a reset of the settings
    ui["btnReset"] = {
        "type": "button",
        "label": "Reset Settings",
        "tooltip": "Resets all settings to their default values",
        "function": "ResetSettings",
        "wsevent": "EVENT_COMMANDPY_SETTINGS_RESET",
        "group": "Actions"
        }
    return json.dumps(ui)

def ResetSettings():
    #resets json/js settings to their defaults
    return
#    UsageMask = 0
...
#  {"Twitch Chat":5,"Twitch Whisper":9,"Discord Chat":6,"Discord Whisper":10,"Both Chat":7,"Both Whisper":11,"Twitch Both":13,"Discord Both":14,"All":15}
